  ...
  @Override
    public AccountUserResponseModel updateAccount(AccountUserRequestModel accountUserRequestModel,String number,  Long userId) {
        ...
        if(accountByNumber.getUser().getId()!=userId){
            ...
            throw new StatusException("You can't update accepted/rejected account");
        }
        else {
   ...
   @Override
    public AccountUserResponseModel deActivate(Long id, Long userId){
        ...
        if(userRepository.getById(userId).getAuthorities().stream().map(authority -> authority.getName()).collect(Collectors.toSet()).contains(AuthorityType.ADMIN)
            ...
            throw new AuthorityException("Please check if you can deActive this Account ((");
        }
